...
SELECT yearid, teamID, w, l , wswin, -- shows w/l of world series winners per year compared to max win (w_max) for that year
	   MAX(w)OVER(PARTITION BY yearid ORDER BY yearid) AS w_max, --w_max which is partitioned by year 
	   CASE WHEN w = MAX(w)OVER(PARTITION BY yearid ORDER BY yearid) THEN 'Y'
			...
			END AS ws_winner_is_max --ws_winner_is_max column that returns 'Y' if the w column matches the w_max column for that year 
FROM teams
WHERE yearid BETWEEN 1970 AND 2016
	AND wswin = 'Y' --filtered for only world series winners per year
--from 1970-2016 the world series winner also had the most wins 100% of the time
